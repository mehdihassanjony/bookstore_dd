...
export class BookRepository extends Repository<Book> {
  getBooks = async (filter: FilterBookDto): Promise<Book[]> => {
    const { title, author, tag, min_year, max_year } = filter;
    ...
    if (tag) {
      query.andWhere('lower(book.tag) LIKE :tag', {
        tag: `%${tag.toLowerCase()}%`,
      });
    }
  ...
  createBook = async (payload: CreateBookDto): Promise<Book> => {
    const { title, author, tag, points, year } = payload;
